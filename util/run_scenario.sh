...
pushd "$(dirname -- "${BASH_SOURCE[0]}")"; cd ..
...
function usage {
  echo "$0 -w WORKLOAD -e EPP [-c CONFIG_FILE]"
  echo "WORKLOAD in $workloads (workload/profiles/inference-perf/workload_*.yaml.in)"
  echo "EPP in $epps (experiments/epp_*.yaml)"
  echo "Optional CONFIG_FILE sourced to set LLMDBENCH ENV (e.g., LLMDBENCH_HF_TOKEN, LLMDBENCH_VLLM_COMMON_PVC_NAME)."
}
...
[ -f ./workload/profiles/inference-perf/${workload}.yaml.in ] || {
  echo "Workload file ./workloads/${workload}.yaml.in does not exist."
  usage
  exit 1
}
if [[ -z $epp ]]; then
...
mkdir -p $LLMDBENCH_CONTROL_WORK_DIR
...
../get_logs.sh $LLMDBENCH_CONTROL_WORK_DIR 2>&1 >$LLMDBENCH_CONTROL_WORK_DIR/log.log &
